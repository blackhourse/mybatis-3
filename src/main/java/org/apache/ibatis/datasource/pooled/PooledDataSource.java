...
public class PooledDataSource implements DataSource {
    ...
    while (conn == null) {
      synchronized (state) {
        ...
        if (!state.idleConnections.isEmpty()) {
          // Pool has available connection 获取首个空闲连接
          ...
          // Pool does not have available connection  比较激活的连接数 与最大激活连接数
          // 激活的连接数 小于 最大激活连接数
          if (state.activeConnections.size() < poolMaximumActiveConnections) {
            // Can create new connection 创建新的PooledConnection连接
            conn = new PooledConnection(dataSource.getConnection(), this);
            if (log.isDebugEnabled()) {
              log.debug("Created connection " + conn.getRealHashCode() + ".");
            }
          } else {
            // Cannot create new connection  不能创建
            // 获取首个激活的 PooledConnection对象
            PooledConnection oldestActiveConnection = state.activeConnections.get(0);
            // 检查该连接是否超时
            long longestCheckoutTime = oldestActiveConnection.getCheckoutTime();
            if (longestCheckoutTime > poolMaximumCheckoutTime) {
              // Can claim overdue connection // 对连接超时的时间的统计
        ...
        // 获取到连接
        if (conn != null) {
          // ping to server and check the connection is valid or not   ping服务器并检查连接是否有效
          if (conn.isValid()) {
            // 如果非自动提交的，需要进行回滚。即将原有执行中的事务，全部回滚。
            // 这里又执行了一次，有点奇怪  TODO
            if (!conn.getRealConnection().getAutoCommit()) {
  ...
  /**
   * Unwraps a pooled connection to get to the 'real' connection     解开池化连接以进入“真实”连接
   *
